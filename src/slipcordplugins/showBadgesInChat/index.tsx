...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        ...
        case "SlipcordContributer":
            return isSlipcordPluginDev(author.id) ? (
                <span style={{ order: settings.store.SlipcordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="Slipcord Contributor"
                        size={20}
                        src={"https://raw.githubusercontent.com/slipcords/Slipper/main/build/icon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, SlipcordDevs.KrystalSkull],
