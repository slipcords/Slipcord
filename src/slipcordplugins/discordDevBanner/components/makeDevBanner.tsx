...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, names, settings,SlipcordIcon } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {slipcordIcon} {slipcordName} {slipcordVersion} ({slipcordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{slipcordVersion}/g, VERSION)
        .replace(/{slipcordHash}/g, gitHashShort)
        .replace(/{slipcordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{slipcordIcon}":
                return <span key={`icon-slipcord-${i}`} className="vc-discord-dev-banner-icons"><SlipcordIcon /></span>;
            case "{electronIcon}":
