...
export function FormatSetting(setValue) {
    ...
    const [state, setState] = useState(format ?? "{buildChannel} {buildNumber} ({buildHash}) | {slipcordName} {slipcordVersion} ({slipcordHash})");
    ...
    return (
        <section>
            ...
            <textarea
                ...
                placeholder="{buildChannel} {buildNumber} ({buildHash}) | {slipcordName} {slipcordVersion} ({slipcordHash})"
                rows={1}
                ref={el => {
                    if (el) {
