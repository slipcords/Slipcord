...
function createSlipcordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Slipcord",
            submenu: [
                {
                    label: "About Slipcord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Slipcord" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair Slipcord",
