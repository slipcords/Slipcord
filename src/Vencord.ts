    ...
    if (!hasCloudAuth) {
        if (Settings.cloud.authenticated) {
            ...
            showNotification({
                ...
                onClick: () => SettingsRouter.openUserSettings("slipcord_cloud_panel")
            });
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("An Slipcord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Slipcord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Slipcord.",
