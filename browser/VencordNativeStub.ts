...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Slipcord/Slipcord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("SlipcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("SlipcordSettings", JSON.stringify(s)),
